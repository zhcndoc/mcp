...
> **实现说明**：在验证整数参数值时，服务器 SHOULD 以数值方式而不是字符串方式比较 header 值和正文值（例如，`42.0` 和 `42` 被视为相等）。
...
前缀 `=?base64?` 和后缀 `?=` 表示该值已进行 Base64 编码。这些标记区分大小写，且 MUST 完全按所示形式出现（小写）。需要检查这些值的服务器和中间件 MUST 相应地对其进行解码。

为避免歧义，客户端 MUST 还要对任何匹配 sentinel 模式的纯 ASCII 值进行 Base64 编码（即以 `=?base64?` 开头并以 `?=` 结尾）。
...
> **注意**：基于镜像 header 执行策略（例如按租户路由或限流）的中间件 SHOULD 验证 `MCP-Protocol-Version` header 是否指示了一个需要 header–body 验证的版本。如果版本较旧或缺少该 header，中间件 SHOULD 拒绝请求，而不是信任未经验证的 header 值。
