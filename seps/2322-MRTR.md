...
2. 服务器返回一个不完整响应，表明客户端需要为请求提供缺失的信息才能成功。
...
上文第 8 步。<b>服务器响应</b> 服务器通过发送 `JSONRPCResultResponse` 确认收到该响应。然而，由于响应缺少必要信息，服务器不会继续处理任务，并将任务状态保持为 `input_required`。客户端下次调用 `tasks/result` 时，服务器会返回一个新的 `inputRequest`，再次请求必要的信息。
...
我们考虑过使用双向流式传输方式来替代 SSE 流。
然而，这种方式会使线路协议更加复杂（例如，需要使用 HTTP/2 或 HTTP/3）。此外，它无法消除不支持长连接的环境中的问题，也无法解决容错问题。

我们曾讨论过输入请求应当是一个映射还是一个单独的对象；也有人建议，可以使用请求中的某个字段（例如，elicitation ID）来区分它们。我们决定使用映射是合理的，因为它在结构上保证了键的唯一性，从而无需在 SDK 和应用程序中进行显式检查来防止冲突。

在持久化工作流中，我们考虑过将输入请求直接包含在 `tasks/get` 响应中，而不是要求客户端先看到 `input_required` 状态，然后再调用 `tasks/result` 获取输入请求。我们决定将二者分开，以便兼容那些为任务状态和实际工具实现使用不同基础设施的实现；其理念是，`tasks/get` 调用应具有一致的延迟特征，并且不应依赖于任务的实际状态。我们意识到，这需要与服务器之间额外进行一次往返，但如果这在未来成为问题，我们可以对其进行优化。
