...
由于这一变更为诸如 'tools/call' 之类的方法调用引入了多态响应，我们在 `Result` 中引入一个新字段来指示 `ResultType`。客户端应解析此字段，以确定消息中包含的 Result 类型。如果未提供此字段，为了向后兼容，客户端应假定 `ResultType` 为 "complete"。架构变更如下：
