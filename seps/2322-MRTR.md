...
- **类型**: Standards Track
- **创建时间**: 2026-02-03
- **作者**: Mark D. Roth (@markdroth), Caitie McCaffrey (@CaitieM20),
...
| ClientRequest           | ServerResult           | InputRequiredResult 支持 |
| ----------------------- | ---------------------- | ----------------------------- |
...
| ClientRequest                  | InputRequiredResult 支持 |
| ------------------------------ | ----------------------------- |
| `PingRequest`                  | 否                            |
| `InitializeRequest`            | 否                            |
| `CompleteRequest`              | 否                            |
...
We considered using a bidirectional streaming approach to replace the SSE stream.
However, this approach would make the wire protocol more complex (for example, it would require HTTP/2 or HTTP/3). In addition, it would not eliminate the issues in environments that cannot support long-lived connections, nor would it solve fault-tolerance issues.

There was discussion about whether the input request should be a map or a single object; it was also suggested that some field in the request (for example, an elicitation ID) might be used to distinguish them. We decided that using a map is reasonable because it structurally guarantees key uniqueness, thereby avoiding the need for explicit checks in the SDK and application to prevent collisions.

In persistent workflows, we considered including the input request directly in the `tasks/get` response, rather than requiring the client to first see an `input_required` state and then call `tasks/result` to obtain the input request. We decided to keep these separate in order to respect implementations that use different infrastructure for task state and the actual tool implementation; the idea is that the `tasks/get` call should have consistent latency characteristics and should not depend on the actual state of the task. We recognize that this requires one additional round trip to the server, but if this becomes a problem, we can optimize it in the future.
