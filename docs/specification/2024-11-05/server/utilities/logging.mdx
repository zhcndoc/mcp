...
Model Context Protocol (MCP) 提供了一种标准化方式，供服务器向客户端发送结构化日志消息。客户端可以通过设置最低日志级别来控制日志记录的详细程度，服务器发送的通知包含严重性级别、可选的记录器名称以及任意的 JSON 可序列化数据。
