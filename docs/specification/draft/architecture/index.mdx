...
- 通过 MCP 基元暴露资源、工具和提示
- 独立运行，职责明确
- 通过回复中的 `InputRequiredResult` 请求客户端输入（采样、询问、根）
- 必须遵守安全约束
- 可以是本地进程或远程服务
...
sequenceDiagram
    ...
    opt 发现
        ...
        Server-->>Client: 支持的版本 + 能力
    end

    loop 客户端请求
        Host->>Client: 用户或模型发起的操作
        Client->>Server: 请求（包含 _meta：版本、clientInfo、clientCapabilities）
        alt 服务器需要客户端输入
            Server-->>Client: InputRequiredResult（例如 sampling/createMessage）
            Client->>Host: 转发给 AI
            Host-->>Client: AI 响应
            Client->>Server: 原始请求（附带输入）
        end
        Server-->>Client: 响应
...
- 已实现的 [服务器功能](/specification/draft/server) 必须在服务器能力中公布
...
- [采样](/specification/draft/client/sampling) 要求客户端在其能力中声明支持
