...
- 错误响应 **必须** 包含 `error` 字段，其中带有 `code` 和 `message`。
- 错误代码 **必须** 为整数。
- 错误响应 **可以** 包含一个 `data` 成员，用于附加任何类型的信息，例如嵌套错误。
...
- 服务器 **不得** 依赖同一连接上的先前请求来建立上下文（例如，能力、协议版本、客户端身份）。每个请求都会在其 [`_meta`](#_meta) 字段中提供这些元数据。
- 服务器 **应该** 准备处理与多个任务、线程或对话相关的请求。
- 服务器 **不应该** 要求客户端重复使用同一连接或进程来执行相关操作。
- 客户端 **不应该** 将单个任务、线程或对话用作 stdio 进程的生命周期边界。
- 需要跨多个请求持续存在的状态（例如，长时间运行的任务、应用程序级句柄）**必须** 通过客户端在每个请求中传递的显式标识符进行引用。
...
<Info>
  有关每请求模型如何映射到 SDK 代码的演练，请参阅
  [架构指南](/docs/draft/learn/architecture#example)。
</Info>
...
`_meta` 属性/参数由 MCP 用于允许客户端和服务器
在其交互中附加额外的元数据。

MCP 为协议级元数据保留了某些键名，如下所述；
实现 **不得** 假定这些键上的值具有任何特定含义。
...
| 键                                           | 描述                                                      | 定义于                                                             |
| -------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------ |
| `progressToken`                              | 将请求纳入进度通知                                        | [进度](/specification/draft/basic/patterns/progress)               |
...
| `io.modelcontextprotocol/logLevel`           | 服务器应为请求发出的最低日志级别                           | [日志](/specification/draft/server/utilities/logging)              |
| `io.modelcontextprotocol/subscriptionId`     | 将通知与其来源订阅关联起来                                  | [订阅](/specification/draft/basic/patterns/subscriptions)          |
| `traceparent`, `tracestate`, `baggage`       | OpenTelemetry 追踪上下文传播                                | OpenTelemetry 追踪上下文（下文）                                    |

官方[扩展](/specification/draft/basic/versioning#extension-negotiation)
会在 `io.modelcontextprotocol/` 前缀下定义额外的 `_meta` 键，
第三方扩展则使用其自己的供应商前缀。
...
标记为必需的字段 **必须** 包含在每个请求中。服务器使用这些字段在不依赖任何先前连接状态的情况下识别所使用的协议版本和能力。有关版本协商规则，请参阅
[版本与兼容性][lifecycle]。

| 键                                          | 类型                 | 必需 | 描述                                               |
| -------------------------------------------- | -------------------- | -------- | --------------------------------------------------------- |
| `io.modelcontextprotocol/protocolVersion`    | `string`             | 是      | 此请求使用的协议版本（例如，`"2026-07-28"`）  |
...
客户端 **应该** 在每个请求中包含 `io.modelcontextprotocol/clientInfo`，
除非经过专门配置而不这样做。

服务器 **不得** 依赖客户端未声明的能力。如果处理请求需要客户端未在
...
服务器 **应该** 在每个结果的 `_meta` 中包含以下 `io.modelcontextprotocol/*` 字段，
除非经过专门配置而不这样做，以便在不依赖任何先前连接状态的情况下标识自身：
...
- `src`：指向图标资源的 URI（必需）。可以是：
  ...
  - 包含 base64 编码图像数据的数据 URI
- `mimeType`：如果源 MIME 类型缺失或过于通用，则可选的 MIME 类型覆盖值
- `sizes`：可选的尺寸规格数组（例如，`["48x48"]`；对于 SVG 等可缩放格式，可以是 `["any"]`；对于多种尺寸，可以是 `["48x48", "96x96"]`）
