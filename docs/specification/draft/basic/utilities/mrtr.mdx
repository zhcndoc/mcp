...
| [`tasks/result`](/specification/draft/basic/utilities/tasks#retrieving-task-results) | 是                           |
...
对于需要服务器端状态的长时间运行操作，持久化工作流利用 [Tasks](/specification/draft/basic/utilities/tasks)。“input_required” 任务状态表示需要额外信息。
...
sequenceDiagram
    ...
    note over C,S: 客户端在服务器处理请求时轮询任务状态

    note over S: 服务器需要通过询问获取更多信息 <br/> 将状态设为 input_required
...
1. 服务器 **MAY** 通过指示任务处于 `input_required` 状态来响应 `tasks/get`。
1. 当任务处于 `input_required` 状态时，服务器 **MUST** 在 `tasks/result` 响应中包含 `inputRequests` 字段。
1. `inputRequests` 的键是服务器分配的标识符，并且在一个 `Task` 的范围内 **MUST** 唯一。

1. 当任务处于 `input_required` 状态时，服务器 **MAY** 在 `tasks/result` 响应中包含 `requestState` 字段。

#### 客户端要求（Tasks）

1. 当 `tasks/get` 显示状态为 `input_required` 时，客户端 **MUST** 调用 `tasks/result` 以获取 `inputRequests` 和可选的 `requestState`。
1. 客户端 **SHOULD** 构造这些请求的结果，并使用 `inputResponses` 和 `requestState`（如果存在）调用 [`tasks/input_response`](/specification/draft/basic/utilities/tasks#providing-input-for-tasks)。
1. 客户端 **MAY** 选择不完成这些输入请求，在这种情况下，客户端 **MAY** 取消任务。
