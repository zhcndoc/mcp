...
参见 [版本管理：向后兼容性][lifecycle-compat]，了解该时代模型
以及面向实现者的兼容性矩阵。

### 更早的 Streamable HTTP 修订版

协议版本 `2025-03-26` 到 [`2025-11-25`](/specification/2025-11-25/basic/transports)
也使用了 Streamable HTTP 传输，但形式不同：服务器可以通过
`Mcp-Session-Id` 头分配会话（通过 HTTP DELETE 终止），客户端可以通过 HTTP GET 打开
一个独立的 SSE 流来接收服务器发起的消息，
服务器可以在 SSE 流上发送 JSON-RPC _请求_，并且流可以通过
`Last-Event-ID` 恢复。这些机制都不属于本修订版。

仅支持此修订版并收到来自旧客户端此类流量的服务器 **SHOULD** 如下响应：

- 向 MCP 端点发送 HTTP GET 或 DELETE：响应
  `405 Method Not Allowed`。
- 请求中的 `Mcp-Session-Id` 头：忽略它，不要生成或回显
  会话 ID。
- `Last-Event-ID` 头：忽略它；流不可恢复。

需要与使用这些协议版本的对端互操作的服务器和客户端，
除了使用上文描述的版本协商回退之外，还应实现对应
修订版中描述的行为（例如，
...
<Warning>
  **已弃用**：来自协议版本
  2024-11-05 的 [HTTP+SSE 传输][http-sse] 自协议版本 `2025-03-26` 起已被弃用，并
  根据 [功能生命周期策略](/community/feature-lifecycle#deprecating-a-feature)
  ...
  HTTP](/specification/draft/basic/transports/streamable-http)。它有望在未来修订版中被移除；请参见
  [已弃用功能注册表](/specification/draft/deprecated)。
</Warning>

客户端和服务器可以按如下方式保持与已弃用的
[HTTP+SSE 传输][http-sse]（来自
协议版本 2024-11-05）的向后兼容：
