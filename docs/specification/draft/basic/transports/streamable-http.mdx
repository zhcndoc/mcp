...
如果服务器因为所需的
`Mcp-Param-*` 头缺失或与正文不匹配而返回
[`HeaderMismatch`](#server-validation) 错误，客户端
**应该** 调用 `tools/list` 以检查工具的
`inputSchema` 是否有变化，然后使用适当的头重试原始请求。

#### 服务器对自定义头的行为

不识别 `Mcp-Param-{Name}` 头的中介服务器
**必须** 转发它，并在其他方面忽略它，正如
[HTTP 语义 RFC][http-semantics] 所要求的那样。
