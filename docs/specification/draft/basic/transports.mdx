...
同时支持现代（按请求元数据）MCP 版本和
需要 `initialize` 握手的旧版本的客户端，**SHOULD** 在发送
任何其他请求之前先通过 [`server/discover`][server-discover] 进行探测。如果服务器返回
`Method not found`（`-32601`），客户端则回退到旧的
`initialize` 握手。如果服务器返回
`UnsupportedProtocolVersionError`，则表示它使用的是不带
`initialize` 的 MCP 版本——客户端 **SHOULD** 使用其公布的
`supportedVersions` 中的某一个重试，而不是回退到 `initialize`。详见
...
- 服务器 **MAY** 发送 JSON-RPC _notifications_——例如
  ...
  这些通知会在最终响应之前发送。这些通知 **MUST** 与
  发起它们的客户端请求相关。
- 服务器 **MUST NOT** 在此流上发送独立的 JSON-RPC _requests_。
  服务器到客户端的交互（采样、询问、列根）会作为输入请求嵌入到
  一个 [`IncompleteResult`][incomplete-result] 中，
  依据 [SEP-2322 (MRTR)][sep-2322]，而不是作为单独请求
  在此流或任何其他流上投递。
- 最终的 JSON-RPC _response_ **SHOULD** 终止该流。

长生命周期的通知流可通过发送
[`subscriptions/listen`][subscriptions-listen]
请求获得。服务器的响应本身就是一个保持打开的 SSE 流，并为客户端选择订阅的通知类型投递
`notifications/tools/list_changed`、
`notifications/resources/updated`、`notifications/message` 等。
...
对于需要在连接中断后仍具持久性的工作负载，请使用
[tasks primitive][tasks]；不支持通过 `Last-Event-ID` 恢复的 SSE
流。
...
[incomplete-result]: /specification/draft/schema#inputrequiredresult
[sep-2322]: /seps/2322-MRTR
[subscriptions-listen]: /specification/draft/basic/utilities/subscriptions
[tasks]: /specification/draft/basic/utilities/tasks
...
（`Method not found`）的 JSON-RPC 错误。

为了向后兼容，如果服务器未收到
`MCP-Protocol-Version` 头且没有其他方式识别版本，
服务器 **SHOULD** 假定协议版本为 `2025-03-26`。
...
同时支持现代（按请求元数据）MCP 版本和
需要 `initialize` 握手的旧版本的客户端，**MAY** 通过先尝试现代请求来检测服务器实现的是哪个时代的协议。如果
服务器返回 `400 Bad Request`（或任何其他表明
服务器未实现现代协议的版本错误），客户端会回退到
`initialize`，并在后续请求中继续使用旧版本。详见
[Lifecycle: Backward Compatibility][lifecycle-compat]
。
...
1. 接受来自用户的 MCP 服务器 URL，该 URL 可能指向使用旧传输或
   新传输的服务器。
2. 尝试向服务器 URL 发起 POST 请求，并使用上文定义的 `Accept` 头：
   - 如果成功，客户端可以假定这是一个支持
     新 Streamable HTTP 传输的服务器。
   - 如果失败并返回 HTTP 状态码 "400 Bad Request"、"404 Not Found" 或 "405 Method Not Allowed"：
     - 向服务器 URL 发起 GET 请求，期望这会打开
       SSE 流并返回一个作为第一个事件的 `endpoint` 事件。
     - 当 `endpoint` 事件到达时，客户端可以假定这是一个运行
       旧 HTTP+SSE 传输的服务器，并应在后续所有通信中使用该
       传输。
