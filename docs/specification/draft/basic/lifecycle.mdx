...
- **HTTP.** 直接尝试发送现代请求。如果服务器返回 `400 Bad Request`（或其他表明服务器未实现现代协议的版本错误），则回退到 `initialize`，并在后续请求中继续使用旧版协议。
- **STDIO.** 由于没有按请求状态码可用于驱动回退，支持两个时代的客户端**应当**先探测 [`server/discover`](/specification/draft/server/discover)，并在 `_meta` 中设置其首选的现代版本。如果服务器返回 `Method not found`（`-32601`），则回退到旧版 `initialize` 握手。如果服务器返回 `UnsupportedProtocolVersionError`，则说明服务器运行的是一种不使用 `initialize` 的 MCP 版本——应使用其公布的 `supportedVersions` 之一，而不是回退到 `initialize`。
