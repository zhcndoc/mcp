...
- **[Tasks](/extensions/tasks/overview)**：用于长时间运行操作的异步执行，支持轮询、运行中输入和持久句柄
- **[Skills over MCP](/community/skills-over-mcp/charter)**：为代理工作流提供丰富、结构化的指令，通过 MCP 发现和使用
- **[MCP Apps](/extensions/apps/overview)**：在对话中内联渲染的交互式 UI 元素（图表、表单、视频播放器）
