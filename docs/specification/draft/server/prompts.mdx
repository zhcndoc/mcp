...
当可用提示词列表发生变化时，声明了 `listChanged` 能力的服务器**应该**向已打开带有 `promptsListChanged: true` 的 [`subscriptions/listen`](/specification/draft/basic/utilities/subscriptions) 流的客户端发送通知：
