...
模型上下文协议（MCP）支持对列表和资源读取结果进行缓存。这使客户端能够缓存响应并减少不必要的重复获取。
缓存与[变更通知](#interaction-with-notifications)是互补的——两种机制可以共存。
...
- <b>生存时间（TTL）字段</b>，`ttlMs`，表示客户端 MAY 认为结果保持新鲜的时长。
- <b>缓存范围字段</b>，`cacheScope`，表示缓存响应的预期作用范围，可以是 `"public"` 或 `"private"`。
...
- should 确保 `cacheScope` 正确反映原语的预期可见性。
