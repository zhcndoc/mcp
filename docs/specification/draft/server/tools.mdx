...
- `inputSchema`: 定义预期参数的 JSON Schema
  ...
  - 如果没有 `$schema` 字段，则默认为 2020-12
  - **MUST** 是一个有效的 JSON Schema 对象（不是 `null`）
  - 对于没有参数的工具，请使用以下有效方法之一：
    - `{ "type": "object", "additionalProperties": false }` - **推荐**：显式只接受空对象
    - `{ "type": "object" }` - 接受任意对象（包括带属性的对象）
  - 属性 **MAY** 包含 [`x-mcp-header`](#x-mcp-header) 注解，以便将参数值暴露为 HTTP 头
- `outputSchema`: 定义预期输出结构的可选 JSON Schema
  - 遵循 [JSON Schema 使用指南](/specification/draft/basic#json-schema-usage)
  - 如果没有 `$schema` 字段，则默认为 2020-12
- `annotations`: 描述工具行为的可选属性
- `execution`: 描述与执行相关属性的可选对象
  - `taskSupport`: 指示此工具是否支持 [任务增强执行](/specification/draft/basic/utilities/tasks#tool-level-negotiation)。取值：`"forbidden"`（默认）、`"optional"` 或 `"required"`
...
The `x-mcp-header` 扩展属性允许服务器在使用 [Streamable HTTP transport](/specification/draft/basic/transports#custom-headers-from-tool-parameters) 时，将特定工具参数镜像到 HTTP 头中。
...
- **MUST** 在 `inputSchema` 中所有 `x-mcp-header` 值之间按大小写不敏感方式唯一
...
服务器开发者 **SHOULD NOT** 使用 `x-mcp-header` 标记敏感参数（密码、API 密钥、令牌、PII），因为头值对网络中介可见。
