...
`listChanged` 是可选的&mdash;服务器可以省略它：
...
服务器 **也可以** 对 `resources/read` 返回一个 [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult)，以表明在读取资源之前需要额外输入。这遵循 [多轮往返请求](/specification/draft/basic/utilities/mrtr#multi-round-trip-requests) 机制。在重试请求时，客户端会在请求参数中包含 `inputResponses`，以及（如果服务器提供了）`requestState`。

或者，如果 `uri` 的 scheme 是 `https://`，客户端可以直接从 Web 获取该资源。有关更多信息，请参见 [Common URI Schemes 部分](#https%3A%2F%2F)。
