...
为了信任、安全和安全性，**应当** 始终有人类介入循环，并拥有拒绝采样请求的能力。
...
支持采样的客户端 **必须** 在 [初始化](/specification/2025-11-25/basic/lifecycle#initialization) 期间声明 `sampling` 能力：
...
4. 根据需要重复多次（服务器可能会限制最大迭代次数，例如在最后一次迭代中传递 `toolChoice: {mode: "none"}` 以强制最终结果）。
...
6. 服务器 **MUST** 确保在回复 `stopReason: "toolUse"` 时，每个 `ToolUseContent` 项都响应一个具有匹配 `toolUseId` 的 `ToolResultContent` 项，并且用户消息仅包含工具结果（无其他内容类型）
7. 双方 **SHOULD** 为工具循环实施迭代限制
