...
When making requests to an MCP server, access token handling **must** comply with the requirements defined in [OAuth 2.1 Section 5, "Protected Resource Requests"](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-5).
Specifically:

1. MCP clients **must** use the Authorization request header field defined in [OAuth 2.1 Section 5.1.1](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-5.1.1):

```
Authorization: Bearer <access-token>
```

Please note that authorization **must** be included in every HTTP request from the client to the server, even if they are part of the same logical session.
...
As an OAuth 2.1 resource server, the MCP server **must** validate access tokens as described in [OAuth 2.1 Section 5.2](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-5.2).
The MCP server **must** verify that the access token was issued specifically for it as the intended audience, according to [RFC 8707 Section 2](https://www.rfc-editor.org/rfc/rfc8707.html#section-2).
If validation fails, the server **must** respond according to the error handling requirements in [OAuth 2.1 Section 5.3](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-5.3). Invalid or expired tokens **must** receive an HTTP 401 response.

MCP clients **must not** send to the MCP server any token other than those issued by the MCP server's authorization server.

MCP servers **must** accept only tokens valid for their own resources.

MCP servers **must not** accept or transmit any other tokens.
...
客户端 ID 元数据文档本身无法防止 `localhost` URL 冒充。攻击者可以通过以下方式声称是任何客户端：
