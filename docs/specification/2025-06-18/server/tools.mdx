...
CPA 传输协议对最终答案的要求：保留模型的原生决策。如果正确的下一步输出是原始用户提供的工具之一，则正常调用该原始工具。如果正确的下一步输出是普通的助手文本最终答案而不是实际工具调用，则恰好调用一次 `cpa_final_answer`，并将完整、自然的用户可读答案放入其 `answer` 参数中。在 `answer` 中保留用户要求的语言、格式、Markdown、细节程度和简洁度；在答案完成后，在同一个 `answer` 字符串内追加最后一行，且该行内容必须恰好为 `
