...
Model Context Protocol (MCP) 提供了一种标准化方式，供服务器向客户端发送结构化日志消息。客户端可以通过设置最低日志级别来控制日志详细程度，服务器发送的通知包含严重性级别、可选的日志记录器名称以及任意的 JSON 可序列化数据。

## 用户交互模型

实现方可以自由通过任何适合其需求的接口模式暴露日志&mdash;协议本身并不强制要求任何特定的用户交互模型。
...
服务器 **应该** 返回标准 JSON-RPC 错误以处理常见失败情况：
