...
Model Context Protocol (MCP) 为服务器提供了一种标准化方式，以便为提示词（prompts）和资源 URI 提供参数自动补全建议。这使得用户能够在输入参数值时接收上下文建议，从而实现丰富的、类似 IDE 的体验。
...
然而，实现方可以自由通过任何适合其需求的界面模式来暴露补全功能——协议本身并不强制要求任何特定的用户交互模型。
...
对于具有多个参数的提示词或 URI 模板，客户端应在 `context.arguments` 对象中包含之前的补全信息，以便为后续请求提供上下文。
