...
如需特定实现细节，请参阅你所使用的 [特定语言 SDK](/docs/2025-06-18/sdk) 的文档。
...
- [MCP SDK](/docs/2025-06-18/sdk)：用于不同编程语言、实现 MCP 的 SDK。
...
MCP 是一种 <Tooltip tip="MCP 的一个子集可以使用 Streamable HTTP 传输变为无状态">有状态协议</Tooltip>，需要进行生命周期管理。生命周期管理的目的是协商客户端和服务器都支持的 <Tooltip tip="客户端或服务器支持的功能和操作，例如工具、资源或提示词">能力</Tooltip>。详细信息可以在 [规范](/specification/2025-06-18/basic/lifecycle) 中找到，而 [示例](#example) 展示了初始化序列。
  ...
  ```json Tool Call Response
  {
    ...
    "result": {
      "content": [
        {
          "type": "text",
          "text": "旧金山当前天气：68°F，局部多云，西风 8 mph，微风。湿度：65%"
        }
      ]
    }
  }
