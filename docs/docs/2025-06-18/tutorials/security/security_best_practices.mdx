...
[MCP 授权](/specification/2025-06-18/basic/authorization)
规范。本文档识别了 MCP 实现中特有的安全风险、攻击向量和最佳实践。
...
1. 用户通过 MCP 代理服务器正常完成认证，以访问  
   第三方 API
2. 在此流程中，第三方授权服务器在用户代理上设置一个 Cookie，  
   表示对静态客户端 ID 的同意
3. 攻击者随后向用户发送包含  
   精心构造的授权请求的恶意链接，该请求包含恶意重定向 URI，  
   以及新的动态注册客户端 ID
4. 当用户点击该链接时，其浏览器仍保留  
   来自之前合法请求的同意 Cookie
...
6. MCP 授权码被重定向到攻击者的服务器  
   （由 [动态客户端注册](/specification/2025-06-18/basic/authorization#dynamic-client-registration) 中恶意 `redirect_uri` 参数指定）
7. 攻击者在未经用户明确批准的情况下，将窃取的授权码兑换为  
   MCP 服务器的访问令牌
8. 攻击者现在可以作为已被攻陷的  
   用户访问第三方 API
...
中被明确禁止，因为它会带来许多安全风险，包括：

- **安全控制规避**
  - MCP 服务器或下游 API 可能实施重要的安全控制，如速率限制、请求验证或流量监控，这些控制依赖于令牌受众或其他凭据约束。如果客户端可以直接获取并使用令牌与下游 API 通信，而 MCP 服务器未正确验证这些令牌，或未确保令牌是为正确的服务颁发的，它们就会绕过这些控制。
- **责任和审计跟踪问题**
  - 当客户端使用上游颁发的访问令牌调用时，MCP 服务器将无法识别或区分 MCP 客户端，该令牌对 MCP 服务器可能是不透明的。
  - 下游资源服务器的日志可能显示，请求似乎来自具有不同身份的不同来源，而不是实际转发令牌的 MCP 服务器。
  - 这两个因素都会使事件调查、控制和审计变得更加困难。
  - 如果 MCP 服务器在未验证令牌声明（例如角色、权限或受众）或其他元数据的情况下传递令牌，拥有被盗令牌的恶意行为者就可以将服务器作为数据泄露的代理。
- **信任边界问题**
  - 下游资源服务器将信任授予特定实体。此信任可能包括关于来源或客户端行为模式的假设。破坏此信任边界可能导致意外问题。
  - 如果令牌被多个服务接受而未进行适当验证，攻击者破坏一个服务后可以使用该令牌访问其他相连的服务。
- **未来兼容性风险**
  - 即使 MCP 服务器今天开始作为“纯代理”，它以后也可能需要添加安全控制。从适当的令牌受众分离开始，更容易演进安全模型。
...
1. 攻击者获取现有会话 ID，并使用该会话 ID 向 **服务器 B** 发送恶意
   事件。
   - 当服务器支持
     [重放/可恢复流](/specification/2025-06-18/basic/transports#resumability-and-redelivery) 时，
     在未收到响应前故意终止请求
     可能导致原始客户端通过用于服务器发送事件的 GET
     请求恢复该请求。
   - 如果某个特定服务器因工具调用而启动服务器发送事件，例如
     `notifications/tools/list_changed`，并且有可能影响
     服务器提供的工具，客户端可能最终会拥有
     它们并不知道已启用的工具。
...
在 OAuth 授权流程期间，MCP 服务器会提供授权 URL，由客户端在浏览器中打开或以编程方式处理。恶意服务器可以通过以下攻击向量利用 MCP 客户端中不足的 URL 验证：
...
4. 攻击者获得客户端应用程序中的 JavaScript 执行上下文，可能导致会话劫持、凭据窃取或进一步利用
...
sequenceDiagram
    ...
    System-->>Proxy: 命令输出
    Proxy-->>Client: 命令结果
    Client-->>MaliciousMCP: 窃取数据/建立持久化
```
...
- **远程代码执行（RCE）**。通过 shell 执行发生命令注入后，攻击者可在用户权限下运行任意代码。
- **权限提升**。XSS 结合 `stdio` 传输可将基于 Web 的攻击升级为对整个系统的完全控制。
- **数据窃取**。攻击者可访问存储在用户系统上的敏感数据、配置文件和凭据。
- **持久化**。攻击者可安装恶意软件、创建后门或修改系统配置，以便长期访问。
...
MCP 客户端 **必须** 在打开 URL 时避免执行 shell：
...
MCP 客户端 **必须** 对从 MCP 服务器接收的所有 URL 进行清理和验证：
...
`stdio` 传输本身并非固有脆弱。然而，在代理架构中，如果一个独立的代理服务管理 `stdio` 连接，并且可以将 MCP 服务器作为子进程启动，那么它就可能提供一条从基于 Web 的攻击升级到完全系统控制的关键路径。

#### 攻击描述

**重要**：此攻击向量仅适用于使用代理架构的 MCP 实现，不适用于直接使用 `stdio` 传输的场景。

在基于代理的 MCP 实现中，本地代理服务位于客户端与 MCP 服务器之间，通过 `stdio` 传输将服务器作为子进程启动。当与客户端侧漏洞结合时，这种架构会形成一条特权升级路径：

1. 攻击者通过 XSS 或其他客户端侧代码执行（例如，通过 OAuth URL 漏洞）获得执行能力
2. 利用上面的攻击向量，恶意行为者从客户端环境中访问客户端与代理之间建立的 MCP 代理认证令牌
...
- **认证绕过**。被盗的代理认证令牌允许未授权访问 stdio 进程启动能力
- **系统入侵**。攻击者可以执行 MCP 代理进程有权限运行的任何命令

#### 缓解措施

主要防御是阻止能够使此攻击向量成立的各类漏洞：

- 实施在[OAuth 授权 URL 验证](#oauth-authorization-url-validation)中描述的缓解措施
...
MCP 代理服务 **SHOULD** 为 `stdio` 传输实施额外的安全控制：

- 为生成的进程实施沙箱或容器化
- 限制生成的 MCP 服务器的文件系统访问
- 记录所有 `stdio` 传输的使用情况以便进行安全监控
- 对潜在危险命令要求额外授权
