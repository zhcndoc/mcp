...
前往 Keycloak 仪表板中的 **客户端作用域**，并创建一个新的 `mcp:tools` 作用域。我们将使用它来访问 MCP 服务器上的所有工具。
...
创建作用域后，请确保将其类型设置为 **默认**，并将 **包含在令牌作用域中** 开关打开，因为令牌验证会需要它。

现在我们还要为 Keycloak 签发的令牌设置一个 **受众**。配置受众很重要，因为它会将预期目标直接嵌入到签发的访问令牌中。这有助于你的 MCP 服务器验证它收到的令牌是否确实是为它准备的，而不是为其他 API 准备的。这对于避免令牌透传场景至关重要。

为此，打开你的 `mcp:tools` 客户端作用域，点击 **映射器**，然后点击 **配置新映射器**。选择 **受众**。
...
上面的受众配置用于测试。对于生产场景，还需要额外的设置和配置，以确保已签发令牌的受众被正确约束。具体来说，受众需要基于从客户端传入的 resource 参数，而不是固定值。
...
最后，我们需要注册一个新的客户端，供 **MCP 服务器本身** 使用，以便与 Keycloak 进行诸如 [令牌自省](https://oauth.net/2/token-introspection/) 之类的交互。为此：

1. 前往 **客户端**。
1. 点击 **创建客户端**。
1. 为你的客户端提供一个唯一的 **客户端 ID**，然后点击 **下一步**。
1. 启用 **客户端身份验证**，然后点击 **下一步**。
1. 点击 **保存**。

值得注意的是，令牌自省只是验证令牌的可用方法之一。也可以借助针对每种语言和平台的独立库来完成。

打开客户端详情后，前往 **凭据** 并记下 **客户端密钥**。
...
在服务器文件夹的根目录下，创建两个文件 `Program.cs` 和 `ProtectedMcpServer.csproj`，以及一个 `Tools` 文件夹。将以下内容填入 `Program.cs`：
...
将以下内容填入 `ProtectedMcpServer.csproj`：
...
在 `Tools` 文件夹中创建 `MathTools.cs`，并填入以下内容：
...
public sealed class MathTools
{
    ...
    public Task<double> Add(
        [Description("第一个操作数")] double a,
        [Description("第二个操作数")] double b)
    {
        return Task.FromResult(a + b);
    }

    [McpServerTool, Description("将两个数字相乘。")]
...
更多详细信息，请参阅 [C# SDK 文档](https://github.com/modelcontextprotocol/csharp-sdk)。
