...
不确定？在 [Discord](/community/communication#discord) 中询问。
...
3. **更新 SEP 编号**：一旦创建了您的 PR，使用 PR 编号重命名文件（例如，PR #1850 变为 `1850-your-feature-title.md`）并更新 SEP 头部。
...
9. **最终确定**：一旦接受，参考实现必须完成。对于具有可观察协议行为的标准轨道 SEP，还必须合并一个[一致性测试](#conformance-test-requirement)。作者将规范更改（模式更改、规范文本和变更日志条目）添加到 SEP 的拉取请求中。SEP 无需 SDK 实现即可变为 `final`。完成这些工作后，赞助人将状态更新为 `final`，SEP 的 PR 即可合并。
...
必须在 SEP 达到“最终”状态之前完成，但在接受之前不必完成。
...
SEP 被接受后，作者将规范更改（模式更改、规范文本和变更日志条目）添加到 SEP 的拉取请求中。参考实现必须完成，同时还要完成任何所需的[一致性测试](#conformance-test-requirement)。不要求 SDK 实现。完成这些工作后，状态变为“Final”，PR 即可合并。
...
对于尚未达到 `final` 状态的 SEP，请直接在该 SEP 的拉取请求中发表评论。

最终版 SEP 作为已接受设计的历史记录予以保留。它们在最终确定后不会更新。如果规范在某个 SEP 达到 Final 状态后发生变化，则以当前规范为准。每个最终版 SEP 页面都会显示相关说明。
