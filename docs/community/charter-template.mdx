...
每个 MCP 工作组和兴趣组都必须维护遵循此结构的章程文档。章程存储在 [modelcontextprotocol 仓库](https://github.com/modelcontextprotocol/modelcontextprotocol) 的 `docs/community/<group-name>/charter.mdx` 处，并添加到 `docs/docs.json`。

章程捕获特定于您的组的信息。治理规则——领导要求、决策流程、会议要求、升级路径——在 [工作组和兴趣组](/community/working-interest-groups) 文档中定义并自动适用。不要在此处重复它们。

标记为 **(仅工作组)** 的部分是工作组必需的，对于兴趣组是可选的。

<Note>
  将下面的 markdown 复制到 `docs/community/<group-name>/charter.mdx` 并替换占位符文本。
</Note>
