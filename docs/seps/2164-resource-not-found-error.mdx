---
title: "SEP-2164: 标准化资源未找到错误码"
sidebarTitle: "SEP-2164: 标准化资源未找到错误代码…"
description: "标准化资源未找到错误码"
---
...
当前 MCP 规范 [建议使用 `-32002`](https://modelcontextprotocol.io/specification/draft/server/resources#error-handling) 作为资源未找到时的错误码。然而，`-32002` 属于 JSON-RPC 的“服务器错误”范围（`-32000` 到 `-32099`），该范围保留给由实现定义的错误，而不是协议级语义。此外，SDK 实现并不一致——6 个官方 SDK 中只有 4 个使用 `-32002`，而 TypeScript SDK 使用 `-32602`，Python SDK 使用 `0`。

本 SEP 将 `-32602`（无效参数）标准化为该场景下正确的 JSON-RPC 错误码，并使规范与 JSON-RPC 标准保持一致。

## 动机

当前 SDK 实现对于资源未找到的错误处理各不相同：
...
这种不一致意味着客户端无法可靠地在不同实现之间检测资源未找到的情况。在 8 个内置资源处理的 SDK 中，使用了四种不同的错误码：`-32002`（C#、Rust、Java、Go、PHP）、`-32602`（TypeScript）、`-32603`（Kotlin）以及 `0`（Python）。Ruby 和 Swift 将错误处理留给服务器实现者。需要区分“资源未找到”和其他错误的客户端必须处理所有这些变体。
...
- 根据 JSON-RPC 规范，`-32000` 到 `-32099` 范围内的自定义代码是“保留给由实现定义的服务器错误”，而不是协议级语义
- 添加一个协议定义的自定义代码要求所有客户端都更新以识别它
...
1. SDK 应将其资源未找到错误码更新为 `-32602`
2. 在过渡期间，客户端应同时将 `-32602` 和 `-32002` 视为资源未找到
3. 规范应将 `-32602` 记录为标准错误码

## 安全影响

无。此更改仅影响错误码值，不影响访问控制或数据暴露。
