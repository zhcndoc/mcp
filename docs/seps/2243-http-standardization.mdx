...
| ------------- | --------------------------------------------------------------------------------- |
| **SEP**       | 2243                                                                              |
| **标题**      | 用于可流式 HTTP 传输的 HTTP 头标准化                                               |
| **状态**      | 最终版                                                                            |
| **类型**      | 标准轨道                                                                          |
| **创建日期**  | 2026-02-04                                                                        |
| **作者**      | MCP Transports 工作组                                                             |
| **发起人**    | 无                                                                                |
...
> **实施说明**：在验证整数参数值时，服务器 SHOULD 按数值而不是按字符串比较头值和正文值（例如，`42.0` 和 `42` 被视为相等）。

**大小写敏感性**：头名称（在 [RFC 9110](https://datatracker.ietf.org/doc/html/rfc9110#name-field-names) 中称为“字段名”）不区分大小写。客户端和服务器 MUST 对头名称使用不区分大小写的比较。
...
使用 Streamable HTTP 传输的客户端 MUST 拒绝任何 `x-mcp-header` 值违反这些约束的工具定义。拒绝意味着客户端 MUST 将无效工具从 `tools/list` 的结果中排除。客户端 SHOULD 在拒绝工具定义时记录警告，包括工具名称和拒绝原因。此行为可确保单个格式错误的工具定义不会阻止其他有效工具的使用。使用其他传输方式（例如 stdio）的客户端 MAY 完全忽略 `x-mcp-header` 注解。
...
前缀 `=?base64?` 和后缀 `?=` 表示该值已进行 Base64 编码。这些标记区分大小写，并且 MUST 按照所示形式精确出现（小写）。需要检查这些值的服务器和中介 MUST 相应地对其解码。
...
> **实现说明**：如果客户端没有工具的 `inputSchema`（例如尚未调用 `tools/list`）或者缓存的 schema 已过期（例如 TTL 已到期），客户端 SHOULD 在不带自定义 `Mcp-Param-*` 头的情况下发送请求。如果服务器因为缺少必需的自定义头而拒绝该请求，客户端 SHOULD 调用 `tools/list` 以获取当前的 `inputSchema`，然后使用适当的头重试原始请求。客户端 MAY 通过其他方式预加载工具定义（例如来自先前会话或配置），以便在未先调用 `tools/list` 的情况下也能发送头。
...
本提案将请求数据镜像到请求头中，而不是将其编码到 URL 路径中。

**请求头的优势**：

1. **简单性**：所有广泛使用的网络负载均衡器都支持基于 HTTP 请求头的路由
2. **多版本支持**：在客户端和服务器中更容易支持多个 MCP 版本
3. **兼容性**：请求头可与现有的 Streamable HTTP 传输设计配合使用，而无需更改端点结构
4. **值无限制**：请求头值可以包含在 URL 中需要编码的字符（例如 `/`、`?`、`#`）
5. **无 URL 长度限制**：很长的值也可以传输，而不会触及 URL 长度限制

**基于路径路由的优势**：

1. **框架简单性**：许多 Web 框架（Flask、Express、Django、Rails）都内置支持基于路径的路由，配置极少
2. **日志记录**：URL 路径通常默认会被记录，便于调试

**权衡与框架考量**：

| 框架             | 基于请求头的路由                                               | 基于路径的路由                              |
| ----------------- | ------------------------------------------------------------------- | ------------------------------------------------ |
| Flask（Python）   | 需要中间件或装饰器在路由前提取请求头 | 原生支持 `@app.route('/mcp/<method>')` |
| Express（Node.js） | 可通过 `req.headers` 轻松实现，但需要自定义路由逻辑            | 原生支持 `app.post('/mcp/:method')`    |
| Django（Python）  | 需要自定义中间件                                          | 原生 URL 模式                              |
| Go（net/http）    | 可通过 `r.Header.Get()` 轻松实现                                           | 可通过路径模式原生支持                         |
| ASP.NET Core      | 可通过 `[FromHeader]` 属性轻松实现                                   | 可通过路由模板原生支持                       |

对于像 Flask 这样强烈偏好基于路径路由的框架，实现基于请求头的路由需要额外代码：

```python
# Flask 示例：基于请求头的路由需要手动分发
...
尽管某些框架中会增加额外复杂性，但最终仍选择基于请求头的路由，原因如下：

1. **向后兼容**：引入基于路径的路由将要求所有现有 MCP 服务器进行重大更新，并且可能需要同时支持两套端点来支持多个版本。即使 SDK 可以在一定程度上掩盖这些问题，测试、指标等额外运维关注点仍然需要处理。基于请求头的路由只需极少的客户端改动。未选择加入的客户端也仍可正常工作。

2. **基础设施收益大于框架复杂度**：主要目标是让网络基础设施（负载均衡器、代理、WAF）无需解析正文就能路由和处理请求。这一收益与服务器所使用的框架无关。

### 基础设施支持

基于 HTTP 请求头的路由和处理已被以下组件支持：
...
- **可观测性**：大多数可观测性方案都可以从 HTTP 请求头中提取数据

### x-mcp-header 中显式的请求头名称

该设计在 `x-mcp-header` 中使用显式名称值，而不是从参数名推导请求头名称，原因如下：

1. **大小写敏感性不匹配**：请求头名称不区分大小写，但 JSON Schema 属性名区分大小写
2. **字符集限制**：请求头名称仅限 ASCII 字符，而工具参数名可能包含任意 Unicode
3. **简单性**：无需用复杂方案从嵌套属性构造请求头名称

### 在 JSON Schema 中的位置

`x-mcp-header` 扩展直接放置在要镜像的属性的 JSON Schema 内部，而不是放在 schema 外部的单独元数据字段中。该设计选择有几个优点：

1. **共置性**：请求头映射与其影响的属性定义在一起，使其立即清楚哪个参数会被镜像。开发者无需在 schema 与单独的元数据结构之间来回查找。
...
5. **降低复杂度**：单独的元数据结构需要定义一种映射机制（例如 JSON Pointer 或属性路径）来将请求头与属性关联起来，这会增加实现复杂度并带来出错风险。

### 范围：仅适用于工具

`x-mcp-header` 机制目前仅适用于 `tools/call` 请求，因为工具是唯一带有 `inputSchema` 且支持 JSON Schema 扩展关键字的 MCP 原语。资源和提示没有等价的 schema 结构：`resources/read` 只接收一个 `uri`（已通过 `Mcp-Name` 暴露），而 `prompts/get` 将参数定义为一个简单的 `{name, description, required}` 数组，不具备 JSON Schema 可扩展性。若要将自定义请求头映射泛化到这些原语，需要为资源和提示增加类似 `inputSchema` 的定义，这属于更大的规范变更。这被记录为未来可能的扩展。

### 不定义规范层面的请求头大小限制

本规范有意不定义单个请求头值长度、MCP 请求头总大小或自定义请求头数量的限制。请求头纯粹是 HTTP 概念，而 HTTP 本身（[RFC 9110](https://datatracker.ietf.org/doc/html/rfc9110)）并未规定请求头大小限制。常见 HTTP 基础设施会施加自己的限制——从某些服务器的 4–8 KB（例如 Apache 约 8190 字节）到其他系统的 128 KB（例如 Cloudflare）不等——但合适的限制取决于部署环境，而只有服务运营者才能确定这一点。

定义规范层面的限制（例如“省略超过 8192 字节的请求头”）会带来问题：

1. **任意阈值**：无论选择什么数值，对某些部署来说都太低，对另一些则无关紧要。合适的“正确”限制因基础设施而异。
2. **适得其反的省略**：如果客户端因为某个请求头超过规范定义的限制而省略它，那么依赖该请求头进行路由的服务器和中间件就必须解析正文或拒绝请求——这会削弱将值暴露在请求头中的核心目的。
3. **不必要的 SDK 负担**：SDK 维护者需要为一个在实践中很少适用的约束实现并测试限制检查逻辑。
4. **与 HTTP 重复**：服务器和中间件已经会使用标准 HTTP 状态码拒绝过大的请求头（`413 Request Entity Too Large`、`431 Request Header Fields Too Large`），客户端无论如何都必须处理这些响应。

> **给实现者的注记**：服务器、中间件和客户端可以根据各自的部署环境独立对单个请求头大小、MCP 请求头总大小或自定义请求头数量施加限制。服务器应记录其施加的任何限制。客户端应优雅地处理 `413 Request Entity Too Large` 或 `431 Request Header Fields Too Large` 响应。工具作者应将 `x-mcp-header` 注释限制在那些能带来明确基础设施收益的参数上。

### 不安全值的编码方式

对于无法安全表示为普通 ASCII 请求头值的参数值（非 ASCII 字符、前导/尾随空白字符、控制字符），我们考虑了四种编码方式：

1. **Sentinel 包裹（所选方案）**：在同一个 `Mcp-Param-{Name}` 请求头内使用 `=?base64?{value}?=` 前缀/后缀来标识 Base64 编码值。

2. **单独的请求头名称**：为编码值使用不同的请求头名称，例如 `Mcp-ParamEncoded-{Name}`，通过请求头名称而不是值格式来表示编码。
...
| 方案                 | 优点                                                                                                                                     | 缺点                                                                                                                                                                                          |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sentinel 包裹    | 每个参数只需一个请求头名称；常见情况（纯 ASCII）可读性强；中间件无需解码即可基于普通值进行路由 | 带内信号在理论上可能与字面值冲突；每个读取方都必须检查该前缀                                                                                       |
| 单独的请求头名称 | 不存在带内歧义；编码方式可从请求头名称自解释                                                                  | 请求头命名空间翻倍；每个中间件都必须为每个参数检查两个请求头名称；如果两者都存在，还需要冲突规则                                                         |
| 隐式编码    | 最简单的线格式；无需 sentinel 或额外请求头                                                                                      | 中间件需要访问工具 schema 才知道是否解码——这违背了在请求头中暴露值的目的；按参数静态决定无法很好处理混合情况 |
| 始终编码        | 规则最简单；无需条件逻辑或歧义                                                                                        | 纯 ASCII 值会变得不可读；中间件必须解码 Base64 才能检查任何值，这会显著削弱本 SEP 的核心动机                                       |

**结论**：Sentinel 包裹方案提供了最佳权衡。自定义请求头的主要用途是让中间件能够对地区名称、租户 ID 之类简单、可读的值进行路由和过滤——这些值几乎总是纯 ASCII，且永远不会触发 Base64 编码。方案 4 使所有值对中间件都变得不可见。方案 3 在没有工具 schema 访问权限的情况下，使中间件无法区分编码值与字面值。方案 2 消除了带内歧义，但会使请求头命名空间翻倍，要求中间件为每个参数检查两种可能的请求头名称，并在两者同时存在时增加冲突规则。方案 1 中 sentinel 的理论碰撞风险可以忽略不计，因为 `=?base64?...?=` 在实践中成为字面参数值的可能性极低。
...
| 测试用例                  | 输入                    | 预期行为                                      |
| -------------------------- | ------------------------ | ------------------------------------------------------ |
| 标头名称大小写变化 | `mcp-method: tools/call` | 服务器 MUST 接受（标头名称不区分大小写） |
| 标头名称混合大小写     | `MCP-METHOD: tools/call` | 服务器 MUST 接受                                     |
...
| 标头中的多余空白 | `Mcp-Name:  foo `        | `"params": {"name": "foo"}` | 服务器 MUST 接受（按 HTTP 规范去除空白）  |
...
| 测试用例                               | 模式                                                    | 预期行为                                                |
| --------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------- |
| 重复的标头名称（相同大小写）      | 两个属性都使用 `"x-mcp-header": "Region"`            | 客户端 MUST 拒绝工具定义                               |
| 重复的标头名称（不同大小写） | `"x-mcp-header": "Region"` 和 `"x-mcp-header": "REGION"` | 客户端 MUST 拒绝工具定义（不区分大小写唯一性） |
| 标头名称与标准头同名     | `"x-mcp-header": "Method"`                                | 允许（生成的是 `Mcp-Param-Method`，不是 `Mcp-Method`）          |
| 空标头名称                       | `"x-mcp-header": ""`                                      | 客户端 MUST 拒绝工具定义                               |
...
| 测试用例                              | 场景                    | 预期行为          |
| -------------------------------------- | --------------------------- | -------------------------- |
| 带 x-mcp-header 的参数为 null    | `"region": null`            | 客户端 MUST 省略头    |
| 带 x-mcp-header 的参数缺失 | 参数不在 arguments 中  | 客户端 MUST 省略头    |
| 可选参数已提供             | 已提供可选参数 | 客户端 MUST 包含头 |
