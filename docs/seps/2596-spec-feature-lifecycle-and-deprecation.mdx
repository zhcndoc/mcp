...
SDK 特定 API、注册表策略或规范文档本身的修订生命周期（草案、当前版、最终版），后者由
[版本指南][versioning]定义。

请注意，本文档中 “最终版” 一词有两种含义：当一个规范_修订版_被后续版本取代时，它就是最终版（见版本指南）；而一个 _SEP_ 在其状态按照 [SEP 指南][sep-guidelines]推进后也会达到最终版。语境通常可以消歧；如无法消歧，本文将明确写作“SEP 达到最终版”或“最终版修订”。
...
术语 “软弃用” 已废弃。规范中现有的相关用法将依据本策略重新分类为已弃用（见 [过渡](#transition)）。
