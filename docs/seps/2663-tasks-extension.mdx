...
| **类型**      | 扩展轨道                                                                                                                                                      |
| **创建日期**  | 2026-04-27                                                                                                                                                            |
| **作者**      | Luca Chang ([@LucaButBoring](https://github.com/LucaButBoring)), Caitie McCaffrey ([@CaitieM20](https://github.com/CaitieM20))；代表代理工作组 |
| **发起人**    | Caitie McCaffrey ([@CaitieM20](https://github.com/CaitieM20))                                                                                                         |
...
interface Task {
  ...
  /** 任务上次更新时的 ISO 8601 时间戳。 */
  lastUpdatedAt: string;

  /**
   * 从创建时刻起的生存时间（毫秒整数）；若为无限则为 null。
   * 服务器可以在 TTL 到期后丢弃该任务。该值在任务的生命周期内
   * MAY 变化。
   */
  ttlMs: number | null;
...
- `failed`：请求在执行期间因 JSON-RPC 错误而失败。该任务会在 `error` 字段中包含 JSON-RPC 错误详情。此状态 **MUST NOT** 用于非 JSON-RPC 错误。
- `cancelled`：请求在完成前被取消。
...
export interface CompletedTask extends Task {
   ...
   * 任务的最终结果。
   * 其结构与原始请求的结果类型一致。
   * 例如，CallToolRequest 任务会返回 CallToolResult 结构。
   */
  result: JSONObject;
}
...
/**
 * 表示带有可选内联 result/error/inputRequests 字段的任务的联合类型。
...
将多轮往返请求与 task 创建结合使用的服务器实现（例如，在创建 task 之前需要通过 `InputRequiredResult` 获取 elicitation 的工具）**SHOULD** 在返回 `CreateTaskResult` 之前 _同步_ 完成所有 MRTR 往返。

### 任务轮询

客户端通过发送 `tasks/get` 请求来轮询任务的完成情况。

客户端在决定轮询频率时 **SHOULD** 遵守响应中提供的 `pollIntervalMs`。`pollIntervalMs` 在任务的生命周期内 **MAY** 变化。服务器 **MAY** 对轮询频率高于记录的 `pollIntervalMs` 的客户端进行限流。

客户端 **SHOULD** 持续轮询，直到任务达到终态，或直到调用 `tasks/cancel`。客户端 **SHOULD** 将任务 ID 持久化到持久存储，以便在崩溃或重启后恢复轮询。
...
收到 `tasks/get` 请求后，服务器 **MUST** 检查任务的状态并相应响应：

1. 如果状态是 `working`，服务器 **MUST** 返回一个状态为 `working` 的任务对象。
2. 如果状态是 `input_required`，服务器 **MUST** 返回一个状态为 `input_required` 的任务对象，并带有[多轮往返请求](./2322-MRTR.md)中定义的 `inputRequests` 字段。`inputRequests` 字段 **MUST** 包含所有尚未完成、且需要在任务继续之前由服务器发给客户端并完成的请求。
3. 如果状态是 `completed`，服务器 **MUST** 返回一个状态为 `completed` 的任务对象，并在 `result` 字段中包含任务的最终结果。
4. 如果状态是 `cancelled`，服务器 **MUST** 返回一个状态为 `cancelled` 的任务对象。
5. 如果状态是 `failed`，服务器 **MUST** 返回一个状态为 `failed` 的任务对象，并包含执行过程中发生的错误。

```typescript
type GetTaskResult = Result & DetailedTask;
```

响应携带与任务当前状态相匹配的相应响应变体（见[任务状态](#task-状态)）。由于这是 `tasks/get` 请求的标准结果形态，该对象上的 `resultType` 字段 **MUST** 设为 `"complete"`。

如果任务的 `ttlMs` 非空，客户端 **MAY** 将 TTL 视为最后保障：如果在 `createdAt` 加上 `ttlMs` 经过后，任务的可观测状态仍未反映更新，则客户端 **MAY** 认为该任务不再可用。反过来，服务器 **MAY** 在 TTL 到期后的任意时间点将任务标记为 `failed`，并随后在任何时候将其删除。`ttlMs` 的值在任务的生命周期内 **MAY** 变化。
...
客户端发送 `tasks/cancel` 请求以表明其希望取消一个进行中的任务。`notifications/cancelled` 通知 **MUST NOT** 用于任务取消。
...
服务器 **MUST** 以空结果确认该请求。若 `taskId` 不对应已知任务，服务器 **SHOULD** 返回 JSON-RPC 错误。取消处理是 _最终一致_ 的——在确认之后，任务的可观测状态 **MAY** 仍保持 `working`（或其他非终态），并且如果工作在取消生效前已完成，最终 **MAY** 到达一个非 `cancelled` 的终态。

取消是 **协作式** 的：请求表达意图，由服务器决定是否以及何时遵从。服务器没有义务真正停止工作；它仅有义务确认该请求。最终变为 `cancelled` 并不保证一定发生。

客户端在发送取消后 **MAY** 立即删除与该任务相关的全部状态（例如，不再需要保留它已经响应过的 `inputRequests` 键列表）。客户端无需再次轮询 `tasks/get` 来等待任务进入 `cancelled` 状态。
...
要开始监听任务状态通知，客户端会向服务器发送 `subscriptions/listen` 请求，并包含一份其感兴趣的任务 ID 列表（见 [SEP-2575](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2575)）：
...
export interface SubscriptionsAcknowledgedNotification extends Notification {
  ...
  params: {
    notifications: {
      /**
       * 为特定任务 ID 订阅 notifications/tasks。
       */
      taskIds?: string[];
      // 其他现有字段...
    };
...
如果客户端请求任务状态通知，但未声明 `io.modelcontextprotocol/tasks` 扩展能力，服务器 **MUST** 返回指定缺失能力的 JSON-RPC 错误：
...
该通知包含完整的任务对象，使客户端无需轮询 `tasks/get` 方法即可访问完整的任务状态和最终结果。客户端 **MAY** 在订阅任务状态通知之外继续轮询 `tasks/get`，但并非必需。

`notifications/progress` 和 `notifications/message` 通知 **MUST NOT** 在任务的 `subscriptions/listen` 流上发送，并且本规范一般也不支持在任务上使用它们。
