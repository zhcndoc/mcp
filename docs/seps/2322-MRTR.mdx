...
| **类型**      | 标准跟踪                                                                                                               |
| **创建于**    | 2026-02-03                                                                                                             |
| **作者**      | Mark D. Roth ([@markdroth](https://github.com/markdroth)), Caitie McCaffrey ([@CaitieM20](https://github.com/CaitieM20)), |
| **赞助者**    | Caitie McCaffrey ([@CaitieM20](https://github.com/CaitieM20))                                                         |
...
由于这一变更为像 `tools/call` 这样的 method call 引入了多态响应，我们正在向 `Result` 中引入一个新字段，用于指示 `ResultType`。客户端应解析该字段，以确定消息中所包含的 `Result` 类型。如果未提供该字段，为了向后兼容，客户端应假定 `ResultType` 为 `"complete"`。

扩展 **MAY** 添加额外的 `ResultType` 值。支持的 `ResultType` 值集合 **MUST** 由核心协议中定义的集合加上通过 capabilities 公布的任何支持扩展的附加值构成。
...
不过，这种方式会使线路协议更加复杂（例如，它需要 HTTP/2 或 HTTP/3）。此外，它也不会消除那些无法支持长连接的环境中的问题，也无法解决容错方面的问题。
