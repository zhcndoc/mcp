...
| ------------- | ---------------------------------------------------------------------------------- |
| **SEP**       | 2577                                                                               |
| **标题**      | 弃用 Roots、Sampling 和 Logging                                                    |
| **状态**      | 最终版                                                                             |
| **类型**      | 标准轨道                                                                           |
| **创建日期**  | 2026-04-14                                                                         |
| **作者**      | Kurtis Van Gent ([@kurtisvg](https://github.com/kurtisvg))                        |
| **发起人**    | [@kurtisvg](https://github.com/kurtisvg)                                           |
...
最近的一次核心贡献者会议提出了弃用这些特性的建议。此 SEP 将该建议形式化，并给出具体的实施计划。请参见 [讨论 #2536][discussion-2536]。

### 根目录（Roots）

根目录提供关于服务端应作用于哪些目录或文件的“信息性指导”。在实践中：

- **采用率低**：很少有客户端实现根目录支持，也很少有服务端依赖它。[特性支持矩阵][feature-matrix] 显示客户端覆盖率有限。
- **语义模糊**：规范将根目录描述为信息性内容——服务端并不要求必须遵守，这降低了它的实用性。
- **重叠的替代方案**：工作目录上下文可以通过工具参数、资源 URI、服务端配置或环境变量提供——这些方式都更明确。

### 采样（Sampling）

采样允许服务端从客户端请求 LLM 补全。尽管概念上很强大，但其采用一直不理想：

- **实现复杂**：正确实现采样需要有人在环审批、模型选择逻辑、安全性考量，以及（自 SEP-1577 起）工具循环支持。这种复杂性导致客户端采用率较低。
- **采用率低**：[特性支持矩阵][feature-matrix] 显示，尽管该特性自 2024 年 11 月规范起就已可用，但支持采样的客户端很少。
- **直接替代方案**：需要 LLM 能力的服务端可以直接集成 LLM 提供商 API，从而完全掌控模型选择、参数和流式传输。

### 日志（Logging）

日志允许服务端通过协议向客户端发送结构化日志消息：
