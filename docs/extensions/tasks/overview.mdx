...
服务器可以通过 `notifications/tasks/status` 推送状态更新。客户端通过 `subscriptions/listen` 机制选择订阅这些通知。每条通知都携带完整的任务状态，从而无需额外一次 `tasks/get` 往返。
...
MCP Tasks 是 [核心 MCP 规范](/specification) 的一个扩展。宿主支持因客户端而异。
