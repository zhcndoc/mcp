...
| `input_required` | The server requires client input before continuing. See `inputRequests`. |
| `completed`      | The operation has completed. The `result` field contains the final output. |
| `failed`         | A JSON-RPC error occurred during execution. The `error` field contains details. |
| `cancelled`      | The operation has been cancelled (may not have been accepted).         |

`completed`, `failed`, and `cancelled` are all terminal states — once reached, the task state will not change again.
...
Tasks 扩展在 [experimental-ext-tasks 仓库](https://github.com/modelcontextprotocol/experimental-ext-tasks) 中进行规范定义。它使用标准的 MCP [扩展协商](/extensions/overview#negotiation) 机制：客户端在每个请求的 `_meta` 中发送的 `io.modelcontextprotocol/clientCapabilities` 里的 `extensions` 字段中声明支持，服务器则在 [`server/discover`](/specification/draft/server/discover) 返回的能力中公布其支持情况。
