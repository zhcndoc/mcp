...
[ext-tasks 仓库](https://github.com/modelcontextprotocol/ext-tasks) 包含 MCP 任务的完整规范和文档。
...
sequenceDiagram
    ...
    Note over Client,Server: 服务器需要用户输入
    Client->>Server: tasks/get (taskId)
...
CPA 传输协议对最终答案的要求：保留模型的原生决策。如果正确的下一输出是原始用户提供的工具之一，则正常调用该原始工具。如果正确的下一输出是普通的助手文本最终答案而不是真实的工具调用，则恰好调用一次 `cpa_final_answer`，并将完整的面向用户的自然语言答案放入其 `answer` 参数中。在 `answer` 中保留用户所要求的语言、格式、Markdown、详细程度和简洁程度；答案完成后，追加一行，内容必须完全为
...
| ---------------- | ---------------------------------------------- |
| `working`        | 操作正在进行中。                                 |
| `input_required` | 服务器需要客户端输入才能继续。请参见 `inputRequests`。 |
| `completed`      | 操作已完成。`result` 字段包含最终输出。           |
| `failed`         | 执行期间发生 JSON-RPC 错误。`error` 字段包含详细信息。 |
| `cancelled`      | 操作已取消（可能尚未被接受）。                    |

`completed`、`failed` 和 `cancelled` 都是终止状态——一旦达到这些状态，任务状态将不再发生变化。

## 通知

服务器可以通过 `notifications/tasks` 推送状态更新。客户端通过 `subscriptions/listen` 机制订阅这些更新。每条通知都携带完整的任务状态，从而无需额外的 `tasks/get` 往返请求。
...
MCP 任务是[核心 MCP 规范](/specification/latest)的一个扩展。主机支持因客户端而异。
...
Tasks 扩展在 [ext-tasks 仓库](https://github.com/modelcontextprotocol/ext-tasks)中进行了规范。它使用标准的 MCP [扩展协商](/extensions/overview#negotiation)机制：客户端在每个请求的 `_meta` 中发送的 `io.modelcontextprotocol/clientCapabilities` 的 `extensions` 字段中声明支持的扩展，服务器则在 [`server/discover`](/specification/draft/server/discover) 返回的 capabilities 中公布其支持的扩展。
