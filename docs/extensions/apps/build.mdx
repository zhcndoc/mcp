...
```typescript
// server.ts
console.log("Starting MCP 应用服务器...");
...
expressApp.listen(3001, (err) => {
  ...
  console.log("服务器正在监听 http://localhost:3001/mcp");
});
```
...
<html lang="en">
  ...
  <body>
    <p>
      <strong>服务器时间：</strong>
      <code id="server-time">加载中...</code>
    </p>
    <button id="get-time-btn">获取服务器时间</button>
...
app.ontoolresult = (result) => {
  ...
  serverTimeEl.textContent = time ?? "[错误]";
};
